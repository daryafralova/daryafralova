<h1 style="border-bottom: none;">Всем привет! 👋 </h1>

Я Даша, Data Analyst.

Помогаю бизнесу видеть, что работает, а что нет. 
Выстраиваю полный цикл работы с данными: от консолидации данных из различных источников и автоматизации ETL-процессов,  до визуализации метрик в real-time дашбордах.

<div align="center">
<hr style="width:50%; border:0; height:2px; background:#333;">
</div>

### 🛠 Мой стек:
- Анализ данных: PostgreSQL, Python(pandas, requests, selenium, sqlalchemy, numpy, seaborn, matplotlib)
- A/B-тестирование: дизайн экспериментов, выбор метрик, анализ результатов
- BI: DataLens, Power BI
- Продуктовая аналитика: KPI, юнит-экономика, RFM, когортный анализ
- Дополнительно: Airflow, Docker, ClickHouse, Hadoop, PySpark

<div align="center">
<hr style="width:50%; border:0; height:2px; background:#333;">
</div>

### 📁 Data-проекты:<br>
| Сфера  | Ссылка | Описание | Инструменты |
|--------|--------|---------------|-------------|
| A/B Testing | [Репозиторий](ссылка) | Проверка продуктовых и маркетинговых гипотез | Python, статистика |
| Data Analysis  | [Репозиторий](https://github.com/daryafralova/data_projects)| Исследовательский анализ данных и визуализация | Python, pandas, numpy, seaborn, matplotlib |
| Dashboards| [Репозиторий](ссылка) | Коллекция дашбордов | DataLens, PowerBI |
| SQL| [Репозиторий](ссылка) |Расчет бизнес-метрик в PostgreSQL|	PostgreSQL |

<div align="center">
<hr style="width:50%; border:0; height:2px; background:#333;">
</div>

### 📜 Диплом и сертификаты:

[Сертификаты](https://github.com/daryafralova/diploma-and-certificates)

<div align="center">
<hr style="width:50%; border:0; height:2px; background:#333;">
</div>

### 📫 Контакты:
- Telegram: t.me/daryafralova
- Email: daryafralova@gmail.com
<div align="center">
<hr style="width:50%; border:0; height:2px; background:#333;">
</div>











